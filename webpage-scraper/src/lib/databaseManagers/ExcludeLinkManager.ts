...
class ExcludeLinkManager {
    ...
    public async getExcludedLinks(university: string): Promise<string[]> {
        ...
        const result: UrlToExclude[] = await new Promise((resolve, reject) => {
            const query: string = 'SELECT * ' + `FROM ${this.tableName} ` + 'WHERE category = ? OR category = ?';
            const values: string[] = [university, 'global'];

            this.db.all(query, values, function (err: Error | null, rows: UrlToExclude[]) {
                if (err !== null) {
    ...
    public async insertExcludedLinks(urls: string[], category: string): Promise<number> {
        ...
        while (urls.length > 0) {
            ...
            lastAffectedRow = await new Promise<number>((resolve, reject) => {
                ...
                const query: string = `INSERT INTO ${this.tableName} ` + '(url, category) ' + 'VALUES ' + Array(subArray.length).fill('(?, ?)').join();
